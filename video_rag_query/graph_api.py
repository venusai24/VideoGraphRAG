...
class GraphAPI:
    ...
    def __init__(self, manager: Optional[MultiGraphManager] = None):
    ...
    def connect(self) -> None:
        if not self._connected:
            ...
            logger.info("GraphAPI: connected to all three Neo4j instances.")
    ...
    @property
    def _mapping(self) -> GraphConnection:
        return self._mgr.mapping_graph
    ...
    def _traverse_mapping(
        ...
        """Traverse APPEARS_IN or SHARES_ENTITY in the Mapping graph."""
        # APPEARS_IN:  (EntityRef)-[:APPEARS_IN]->(ClipRef)
        # SHARES_ENTITY: (ClipRef)-[:SHARES_ENTITY]->(ClipRef)
        # We try both directions so the caller does not need to worry.
        cypher = (
            "UNWIND $ids AS nid "
            "MATCH (a {id: nid})-[r:" + edge_type + "]->(b) "
            "RETURN a.id AS source, b.id AS target, type(r) AS etype, "
            "       r.weight AS weight, r.confidence AS confidence, "
            "       r.source AS src_label"
        )
        records = self._mapping.execute_query(cypher, {"ids": node_ids})
        edges: List[TraversalEdge] = []
        for r in records:
            w = r.get("weight") or r.get("confidence") or 1.0
    ...
    def _fetch_properties_batch(self, node_ids: List[str]) -> None:
        ...
        # Mapping graph — EntityRef / ClipRef are lightweight reference nodes
        cypher_map_entity = (
            "UNWIND $ids AS nid "
            "MATCH (e:EntityRef {id: nid}) "
            "RETURN e.id AS id, 'EntityRef' AS node_label"
        )
        for r in self._mapping.execute_query(cypher_map_entity, {"ids": node_ids}):
            if r["id"] not in self._property_cache:
                self._property_cache[r["id"]] = {"node_label": "EntityRef"}

        cypher_map_clip = (
            "UNWIND $ids AS nid "
            "MATCH (c:ClipRef {id: nid}) "
            "RETURN c.id AS id, 'ClipRef' AS node_label"
        )
        for r in self._mapping.execute_query(cypher_map_clip, {"ids": node_ids}):
            if r["id"] not in self._property_cache:
                self._property_cache[r["id"]] = {"node_label": "ClipRef"}
