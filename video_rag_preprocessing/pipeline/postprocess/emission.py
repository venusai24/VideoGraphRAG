...
class EmissionBuffer:
    ...
    def push_populated(self, frame: OutputFrame, hist: List[np.ndarray]):
        self.emission_buffer.append(frame)
        self._resolve_emissions(hist)

    def push_empty(self, target_time: float, hist: List[np.ndarray]):
    ...
    def _resolve_emissions(self, hist: List[np.ndarray]):
