...
def read_wav_samples(wav_bytes: bytes) -> Tuple[np.ndarray, int]:
...
def apply_noise_gate(
    ...
    *,
    ...
    pad_len = (window_size - (len(samples) % window_size)) % window_size
    ...
    reshaped[~mask] = 0.0
    gated = reshaped.flatten()
    return gated[:len(samples)]
...
def prepare_audio_input(
    ...
    *,
    ...
    gated = apply_noise_gate(samples, actual_sample_rate) if apply_vad else samples
...
def serialize_audio_to_wav_bytes(audio_input: AudioClipInput) -> bytes:
    ...
    with wave.open(buffer, "wb") as handle:
        ...
        samples = np.asarray(audio_input.audio_array, dtype=np.float32)
